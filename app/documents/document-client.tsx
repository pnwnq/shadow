  ...
  const filteredDocuments = useMemo(() => {
    return documents.filter((doc) => {
      ...
      if (activeCategory) {
        ...
        return doc.category?.id === activeCategory
      }
      return true
    })
                    ...
                    <Card key={doc.id}>
                      ...
                      <CardContent className="pb-2">
                        <div className="flex items-center gap-2 text-sm text-muted-foreground">
                          ...
                          <span>
                            {(doc.category?.name || "未分类")} · {formatBytes(doc.size)}
                          </span>
                        </div>
                      </CardContent>
                ...
                <div className="rounded-md border">
                  ...
                  {filteredDocuments.map((doc) => (
                    <div key={doc.id} className="grid grid-cols-12 gap-2 p-4 border-b hover:bg-muted/50 items-center">
                      ...
                      <div className="col-span-2 text-muted-foreground">{doc.category?.name || "未分类"}</div>
